...
export default function PurchaseTicket({ eventId }: PurchaseTicketProps) {
  ...
  // CALCULATE TIME REMAING ON MOUNT
  useEffect(() => {
    const calculateTimeRemaing = () => {
      if (isExpired) {
      ...
      if (minutes > 0) {
        setTimeRemaining(`${minutes} minutes${minutes === 1 ? '' : 's'} ${seconds} seconds${seconds === 1 ? '' : 's'}`);
      } else {
    ...
    const interval = setInterval(calculateTimeRemaing, 1000);
